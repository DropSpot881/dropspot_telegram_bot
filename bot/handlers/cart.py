...
from bot.config import CRYPTO_WALLET_BTC, CRYPTO_WALLET_ETH, SHOP_NAME
...
async def cart_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, item in enumerate(cart, 1):
        ...
        total += sub
        lines.append(f"*{i}.* {item['product_name']} ({item['variant_name']})\n    {item['quantity']}x · {format_price(item['price'])} = *{format_price(sub)}*")
        buttons.append([
            InlineKeyboardButton(f"🗑 {item['product_name']} ({item['variant_name']})", callback_data=f"rmcart_{item['product_id']}_{item['variant_id']}")
        ])
...
async def remove_from_cart_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await models.remove_from_cart(user_id, product_id, variant_id)
...
async def checkout_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["checkout_items"] = [
        {"product_id": i["product_id"], "name": f"{i['product_name']} ({i['variant_name']})",
         "quantity": i["quantity"], "price": i["price"], "variant_id": i["variant_id"]}
        for i in cart
    ]
...
async def address_entered(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [InlineKeyboardButton("₿ Crypto (BTC/ETH)", callback_data="pay_crypto")],
...
async def checkout_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payment == "crypto":
        pay_text = (
            f"\n₿ *Send {format_price(total)}:*\n\n"
            f"BTC:\n`{CRYPTO_WALLET_BTC}`\n\n"
            f"ETH:\n`{CRYPTO_WALLET_ETH}`\n\n"
            "Tap *I Have Paid* after sending 👇"
        )
        confirm_btn = InlineKeyboardButton("💸 I Have Paid", callback_data=f"ipaid_{order_id}")
    else:
        pay_text = (
