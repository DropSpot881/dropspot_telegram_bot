...
async def vnd_addp_variants_ask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["vnd_addp_desc"] = update.message.text.strip()
    await update.message.reply_text(
        f"{hdr('🔢', 'Step 4: Variants')}\n\n"
        "Enter product variants / deals in this format: ✍️\n\n"
        "`Name for Price, Name for Price`\n\n"
        "_Example:_ `4 for 500, 10 for 800, 20 for 1500`\n\n"
        "This defines the quantity and price for each option.",
        parse_mode="Markdown"
    )
    return ADD_PROD_VARIANTS


async def vnd_addp_methods_ask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    variants_raw = update.message.text.strip()
    parsed_variants = []
    
    # Simple parser: "Name for Price" or "Name: Price" or "Name-Price"
    for part in variants_raw.split(","):
        ...
        for sep in (" for ", ":", "-", "="):
            if sep in part.lower():
                subparts = part.lower().split(sep)
                break
        
        if subparts and len(subparts) >= 2:
            try:
                name = subparts[0].strip().title()
                price = float(subparts[1].strip().replace(" ", "").replace(",", "."))
                parsed_variants.append({"name": name, "price": price})
            except ValueError:
                continue
                
    if not parsed_variants:
        await update.message.reply_text("❌ Could not parse any variants. Please use format: `4 for 500, 10 for 800` ✍️")
        return ADD_PROD_VARIANTS

    context.user_data["vnd_addp_variants"] = parsed_variants
...
async def vnd_addp_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use price of first variant as base price
    base_price = data["vnd_addp_variants"][0]["price"] if data["vnd_addp_variants"] else 0
    
    prod_id = await models.add_product(
    ...
    # Save variants
    for v in data["vnd_addp_variants"]:
        await models.add_variant(prod_id, v["name"], v["price"])
...
async def vnd_editp_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🔢 Edit Variants", callback_data="evp_variants")],
...
async def evp_variants_ask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text(
        "Enter NEW product variants (replaces old ones) ✍️\n\n"
        "Format: `4 for 500, 10 for 800`"
    )
    return VND_EDITP_VARIANTS

async def evp_variants_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    variants_raw = update.message.text.strip()
    parsed_variants = []
    ...
    if not parsed_variants:
        await update.message.reply_text("❌ Could not parse. Use: `4 for 500, 10 for 800` ✍️")
        return VND_EDITP_VARIANTS

    prod_id = context.user_data["editp_id"]
    await models.delete_variants(prod_id)
    for v in parsed_variants:
        await models.add_variant(prod_id, v["name"], v["price"])
        
    # Update base price to lowest variant
    await models.update_product(prod_id, price=parsed_variants[0]["price"])
    
    await update.message.reply_text("✅ Variants updated!")
    return await vnd_products_menu(update, context)
...
def get_vendor_conversation_handler():
    return ConversationHandler(
        ...
        states={
            ...
            ADD_PROD_DESC: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, vnd_addp_variants_ask),
            ],
            ADD_PROD_VARIANTS: [
            ...
            VND_EDITP_MENU: [
                ...
                CallbackQueryHandler(evp_variants_ask, pattern=r"^evp_variants$"),
            ...
            VND_EDITP_VARIANTS: [MessageHandler(filters.TEXT & ~filters.COMMAND, evp_variants_save)],
            VND_EDITP_METHODS: [
